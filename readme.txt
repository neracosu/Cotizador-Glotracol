...
* Los registros ya no se ensucian con eventos de las pruebas internas: los errores de webhook que aparecían sobre cotizaciones reales los provocaba una prueba del propio plugin, no un fallo de envío.
...
* Los productos sin precio cargado ya no desaparecen del correo: se muestran como "A cotizar" y el total se marca como parcial, en lugar de enviar un correo sin ningún precio.
* El correo al cliente ahora incluye los datos que llenó en el formulario (nombre, empresa, NIT, teléfono, ciudad y mensaje).
* Correos rediseñados y unificados: una sola plantilla para el equipo y otra para el cliente, con los avisos de pedido grande y de precios pendientes integrados.
* Nuevo PDF de la cotización, adjunto automáticamente a los correos y descargable desde la ficha de la cotización en el panel.
* Integración con GoHighLevel: nuevo formato de webhook con campos planos y los productos como texto, listos para mapear en una oportunidad.

= 2.12.0 =
* Precios Lista A que sí se cargan: la carga "Precios Lista A" (antes "Lista de precios públicos") ahora escribe el precio real por producto (_glo_price), el mismo que muestra la pantalla de Precios y usa el cotizador. Antes iba a un registro interno por SKU que ninguna pantalla leía. Acepta ID o SKU en la columna.
* Nombres más claros para no confundir: "Precios Lista A (rápido: ID o SKU + precio)" y "Catálogo Lista A completo (ID, nombre, stock…)".
* La carga rápida de Lista A ahora pasa por la previsualización/cotejo con el nombre del producto al lado (ID → nombre → precio actual → nuevo).
* Depurador de archivos: detecta cuando un archivo quedó "corrido" una fila (por una celda de precio vacía que empuja el resto), avisa las líneas afectadas y ofrece un realineado acotado con un clic. También avisa IDs/SKU duplicados y nombres que no coinciden con el catálogo.

= 2.11.0 =
* Cotejo de importación: al subir precios de catálogo o Lista B, la previsualización ahora muestra el valor actual → nuevo de cada producto, con resumen (cambian / nuevos / iguales / sin coincidencia), filtros, y control por fila (incluir / saltar / editar). Alerta cuando la mayoría de los precios bajan (posible Lista B cargada como catálogo).
...
* Corrección de texto: productos sin precio muestran "A cotizar" (antes "A cotizar c/u").

= 2.9.0 =
* El cuadro de cotización ahora muestra columnas Producto, Presentación, Cantidad y Valor, con el Total de la solicitud.
* Precio en vivo por NIT: al escribir su NIT, los clientes de Lista B ven automáticamente sus precios negociados (sello "Precio B2B aplicado"); el público general ve la Lista A.
* Nuevo texto de "Presentación" por producto (ej. "Saco 25 kg"): editable en la ficha del producto (pestaña Presentaciones) o cargable masivamente en la misma hoja del importador de catálogo mediante la columna "presentacion". Si se deja vacío, se muestra el peso del producto.
